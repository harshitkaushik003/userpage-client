...
export const getInitialState = createAsyncThunk('user/get', async (page)=>{
    const response = await axios.get(`https://userpage-api-eta.vercel.app/user?page=${page}`);
    return response.data;
})

export const getSearchResults = createAsyncThunk('user/search', async (arg1)=>{
    const response = await axios.get(`https://userpage-api-eta.vercel.app/user?search=${arg1}`);
    return response.data.data;
})

export const applyFilters = createAsyncThunk('user/filters', async(args)=>{
    const response = await axios.get(`https://userpage-api-eta.vercel.app/user?domain=${args.arg1}&gender=${args.arg2}&available=${args.arg3}`);
    console.log(response);
    return response.data;
})

export const AddNewData = createAsyncThunk('user/add', async(userData)=>{
    const response = await axios.post("https://userpage-api-eta.vercel.app/user", userData, {
        headers:{
